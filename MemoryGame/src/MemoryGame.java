...
public class MemoryGame {
	...
	public static void main(String[] args) {
		...
		//canvas.setVisible(false); // used to hide or show windows
		...
		/*
		 *  Child window CANVAS DOES NOT GET OPEN events!!
		 */
        canvas.addListener(SWT.Show, new Listener() {
			public void handleEvent(Event e) {
				System.out.println("CANVAS OPEN!!!");
        }});
		...
		/* 
		 * This is done only once when the shell opens. Do all initialization here.
		 */
        shell.addListener(SWT.Show, new Listener() {
			public void handleEvent(Event e) {
				...
				int xStartButton = clientArea.width / 2 - 150 / 2;
				int yStartButton = clientArea.height - 70 - 30;
				int xEndButton = clientArea.width / 2 - 150 - 20;
				int yEndButton = clientArea.height - 70 - 30;
				...
				int startingX = (clientArea.width - cardsTotalWidth) / 2;
				int startingY = (clientArea.height - cardsTotalHeight) / 2;
	...
	// Welcome Screen
		static void drawWelcome(PaintEvent e) {
			...
			// welcome text
			Point pt = textCenterXY(e.gc, "Welcome to my Memory Game!", e);
			e.gc.drawText("Welcome to my Memory Game!", pt.x, 50, true); // true for transparency
			
			// instructions background shapes
			Point ptRect = shapeCenterXY(400, 300, e);
			Point ptOval = shapeCenterXY(340, 300, e);
			...
			Point ptInstruct = textCenterXY(e.gc, "Instructions:", e);
			e.gc.drawText("Instructions:", ptInstruct.x, 190, true); 
			
			// instruction details
			e.gc.setFont(new Font(e.display, "Arial", 30, SWT.NORMAL));
			
			Point pt1 = textCenterXY(e.gc, "1. Flip 2 cards", e);
			e.gc.drawText("1. Flip 2 cards", pt1.x, 300, true);
			Point pt2 = textCenterXY(e.gc, "2. Try to get all the matches", e);
		...
		static void drawEndScreen(PaintEvent e) {
			...
			e.gc.fillRectangle(e.width / 2 - 500 / 2, e.height / 2 - 200 / 2, 500, 200);
			e.gc.drawRectangle(e.width / 2 - 500 / 2, e.height / 2 - 200 / 2, 500, 200);
			
			
			// Game Over text
			Point point = textCenterXY(e.gc, "Game Over!", e);
		...
		static Point textCenterXY(GC gc, String text, PaintEvent e) {
	    	Point extent = gc.stringExtent(text);
	    	int textWidth = extent.x;
	    	int textHeight = extent.y;
	    	int x = (e.width - textWidth) / 2;
	    	int y = (e.height - textHeight) / 2;
	    	return new Point(x, y);
	    }
		
		// used to draw shapes exactly in the middle of the screen
		static Point shapeCenterXY(int width, int height, PaintEvent e) {
	    	int x = (e.width - width) / 2;
	    	int y = (e.height - height) / 2 - height / 2;
	    	return new Point(x, y);
	    }
